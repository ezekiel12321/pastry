...
public class BroadcastMessage extends ScribeMessage {
    ...
    public BroadcastMessage(NodeHandle source, Topic topic, ScribeContent content) {
    ...
    public BroadcastMessage(NodeHandle source, Topic topic, RawScribeContent content) {
	    super(source, topic);

	    this.content = content;
	}
 
    // ccw:  due to event-based system, 
    // this priority gives events priority and find one to tackle with highest priority.
    ...
    public static BroadcastMessage build(InputBuffer buf, Endpoint endpoint, ScribeContentDeserializer scd) throws IOException {
	    byte version = buf.readByte();
	    switch(version) {
	      case 0:
	        return new BroadcastMessage(buf, endpoint, scd);
	      default:
     ...
     * version number.
     */
    private BroadcastMessage(InputBuffer buf, Endpoint endpoint, ScribeContentDeserializer cd) throws IOException {
        super(buf, endpoint);
